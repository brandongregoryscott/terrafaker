...
USAGE
  $ terrafaker generate repo [--chaos-tags | --tags <value> | --no-tags] [--count <value>] [--create-remote]
    ...
    [--resource-count <value>] [--vcs-provider github|gitlab]
...
FLAGS
  ...
  --create-remote
      Create and push a remote repo. Requires the `gh` or `glab` CLI to be installed. To install, run `brew install
      gh|glab`.
...
Clones repos from your GitLab account, useful for pulling down generated repos for manual modifications. Requires the `glab` CLI to be installed. To install, run `brew install glab`.
...
DESCRIPTION
  Clones repos from your GitLab account, useful for pulling down generated repos for manual modifications. Requires the
